...
parser.add_argument("--model_path", default="EleutherAI/pythia-6.9b", type=str)
parser.add_argument("--revision", default=None, type=str)
parser.add_argument("--tokenizer_path", default=None, type=str)
parser.add_argument("--dataset", default="Dahoas/full-hh-rlhf", type=str)
parser.add_argument("--total_steps", default=14000, type=int)
...
parser.add_argument("--num_unfrozen_layers", default=16, type=int)
parser.add_argument("--gradient_checkpointing", action="store_true")
parser.add_argument("--load_in_4bit", action="store_true")
parser.add_argument("--downscale_weight", action="store_true")
parser.add_argument("--checkpoint_dir", default="checkpoints/reward_model", type=str)
parser.add_argument("--eval_interval", default=1000, type=int)
parser.add_argument("--only_eval", action="store_true")
parser.add_argument("--default_format", default="no_change", action="store_true")
...
if __name__ == "__main__":
    ...
    accelerator.init_trackers(
        ...
        config=vars(args),
        init_kwargs={"wandb": {
            "name": f"{model_name}@{args.dataset}",
            "entity": "eleutherai",
            "group": "Pythia-6.9b-rm"
        }},
    )

    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_path or args.model_path)

    if args.default_format == 'oasst':
    ...
    if args.default_format == 'oa':
        dataset = dataset.map(to_oa_format)
    elif args.default_format == 'vicuna' :
    ...
    if args.num_unfrozen_layers is not None and args.num_unfrozen_layers > 0:
        ...
        try:
            for layer in model.gpt_neox.layers[:-args.num_unfrozen_layers]:
                layer.requires_grad_(False)
            frozen = True
        except AttributeError:
            pass
    ...
    tbar = tqdm(range(min(args.epochs * len(dataloader), args.total_steps)), disable=not accelerator.is_main_process or args.only_eval)
    for iepoch in range(args.epochs):
        for batch in dataloader:
            ...
            if step >= args.total_steps: # Break from current epoch
                break
        
        if step >= args.total_steps: # Break from other epochs
            break